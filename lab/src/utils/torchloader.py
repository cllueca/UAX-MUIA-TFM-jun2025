...
def load_torch_model_state_dict(
    model_path: str,
    model_architecture: UNet3D | AttentionUnet3D | VisionTransformer,
...
def load_complete_model(model_path: str) -> UNet3D | AttentionUnet3D | VisionTransformer:
    """Load a complete model including architecture and weights"""
    try:
        model = torch.load(model_path, weights_only=False)
    except:
        model = torch.load(model_path.replace('pt', 'pth'), weights_only=False)
...
def load_model_with_params(
    ...
    if class_name in ['UNet3D', 'AttentionUnet3D', 'VisionTransformer']:
        if model_config['model_type'] == 'unet-original':
            ...
            model = VisionTransformer(**init_params)
    else:
